...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("<div class='section-header'>⚙️ Frame Processing Mode</div>", unsafe_allow_html=True)
        st.markdown("<p style='color: #a0aec0; font-size: 0.85rem; margin-bottom: 10px;'>Balance between speed and accuracy</p>", unsafe_allow_html=True)
        
        processing_mode = st.radio(
            "Processing Mode",
            ["🎯 Maximum Accuracy (Slow)", "⚖️ Balanced (Medium)", "⚡ Fast Processing"],
            label_visibility="collapsed"
        )
        
        if "Maximum Accuracy" in processing_mode:
            frame_skip = 1
            st.markdown("<div class='info-box'>📊 Process: <strong>Every Frame</strong><br>Speed: 3-7 FPS | Accuracy: ⭐⭐⭐⭐⭐</div>", unsafe_allow_html=True)
        elif "Balanced" in processing_mode:
            frame_skip = 3
            st.markdown("<div class='info-box'>📊 Process: <strong>1 of 3 Frames</strong><br>Speed: 10-15 FPS | Accuracy: ⭐⭐⭐⭐</div>", unsafe_allow_html=True)
        else:
            frame_skip = 5
            st.markdown("<div class='info-box'>📊 Process: <strong>1 of 5 Frames</strong><br>Speed: 20-25 FPS | Accuracy: ⭐⭐⭐</div>", unsafe_allow_html=True)
    ...
    # -------------------------------------------------------
        ...
        if uploaded_video:
            ...
            if start_video:
                ...
                st.info(f"📊 Video Info: {total_frames} frames @ {fps_video:.1f} FPS")
